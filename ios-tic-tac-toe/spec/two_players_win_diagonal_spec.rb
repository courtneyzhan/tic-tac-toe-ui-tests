...
describe "Simple Two Player Game Ending in Player Diagonal Win" do
  include TestHelper
  ...
  # Bug: find restart does not reset the order. Individual run OK.  Run together, second failed with player text.  
  it "Complete game, Player O Wins by Diagonal /" do
    ...
    game_page.click_tile(8)
    game_page.click_tile(7)
    try_for(3) { expect(game_page.page_header_text).to eq("Player O won!") }
  end
  
  it "Complete game, Player X Wins by Diagonal \\" do
    game_page = GamePage.new(driver, $main_win)
    # X_O 
    # OX_
    # __X
