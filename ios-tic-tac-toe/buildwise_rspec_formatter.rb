  ...
  module RSpec3
    class Formatter
      ...
      def example_failed(notification)
        ...
        current_spec.failures << failure
        ...
        elsif false && $driver && $driver.class.name == "Appium::Core::Base::Driver" 
          # Add screenshots capability for running Appium tests in BuildWise
          ...
          begin
            ...
            # screenshot = $driver.screenshot_as(:base_64)
            # File.write(File.join(saved_to, "#{example_name}.png"), screenshot)
          rescue  => e
            # don't cause build to stop if errors happens
            fio = File.open( File.join(reports_dir, "error.log"), "a")
            fio.puts(e)
